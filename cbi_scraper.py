...
try:
    ...
    articles = soup.select("article.newsitem")
    print("Articles found:", len(articles))

    for article in articles:
        link_element = article.select_one("h1 > a")
        link = urljoin(URL, link_element.get("href"))
        
        title = link_element.get_text(strip=True)
        
        desc_element = article.select_one("p.new-cms")
        desc = desc_element.get_text(strip=True)
        
        date_element = article.select_one("span.date")
        date = format_datetime(
                    datetime.strptime(date_element.get_text(strip=True), "%d/%m/%Y -").replace(hour=0, minute=0, second=0, tzinfo=timezone.utc)
                )
    ...
    SubElement(channel, "title").text = "Home - ChessBase India"
    SubElement(channel, "description").text = "Read every news about chess in detail with ChessBase India in English. Read English news related with Chess such as Tournament, Players, every report of the Match."
    ...
    SubElement(image, "url").text = "https://www.google.com/s2/favicons?domain=https://chessbase.in"
    SubElement(image, "title").text = "Home - ChessBase India"
    ...
    ElementTree(rss).write("feed/cbi.xml", encoding="utf-8", xml_declaration=True)
except Exception as e:
    print("Exception in cbi_scraper")
    print(e)